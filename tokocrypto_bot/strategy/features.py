...
DESCRIPTION: Multi-Pair Deterministic Feature Engineering Engine (P1-B).
"""
...
class FeatureEngine:
    ...
    def compute_features(self, df_klines: pd.DataFrame, symbol: str) -> FeatureFrame:
        ...
        # 1. Zero Look-Ahead Bias: Filter Hanya Closed Candles
        if "is_complete" in df_klines.columns:
            closed_df = df_klines[df_klines["is_complete"] == True].copy()
        else:
            closed_df = df_klines.iloc[:-1].copy()

        # 2. History Minimum Validation
        if len(closed_df) < MIN_REQUIRED_CANDLES:
            err = f"Insufficient history (Required >= {MIN_REQUIRED_CANDLES}, got {len(closed_df)})"
            return FeatureFrame(0, symbol, self.feature_version, {}, False, err)
        ...
        try:
            ...
            # Volatility Regime (20-period Rolling Std of Log Returns)
            ...
            # Directional Movement (DMI: DI+ / DI-)
            ...
            # Ambil candle closed paling akhir (-1)
            idx = -1
            ...
            for col, val in raw_features.items():
                if np.isnan(val) or np.isinf(val):
                    err = f"NaN/Inf in feature [{col}]: {val}"
    ...
    def compute_multi_pair_features(self, klines_map: Dict[str, pd.DataFrame]) -> Dict[str, FeatureFrame]:
        """Memproses sekumpulan DataFrame OHLCV untuk banyak pair sekaligus."""
        results = {}
        for symbol, df in klines_map.items():
            results[symbol] = self.compute_features(df, symbol)
        return results
